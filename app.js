...
const materialsCol = collection(db, "ingredientMaster");
...
const metaListsDoc = doc(collection(db, "metaLists"), "shared");
...
const approverMaterialsCol = collection(approverDb, "ingredientMaster");
...
const DELETE_APPROVER_EMAIL = "kangawin@th-umios.com";
...
let currentUser = null; // Firebase User, set by onAuthStateChanged
...
function requestAuthConfirm(title, message, onSuccess, options = {}){
...
function renderRefListItems(){
  ...
  const saveEdit = row => {
    ...
    const extra = isCustomers ? { country: newCountry, image: refListEditingCustomerImage, locations: newLocations } : (isProductTypes ? { code: newCode } : (isContacts ? newContact : {}));
    ...
    if(!newVal || (newVal === item.name && (!isCustomers || newCountry === (item.country || '')) && imageUnchanged && contactUnchanged && locationsUnchanged)){
      refListEditingId = null; refListEditingCustomerImage = null; refListEditingLocations = []; renderRefListItems(); return;
    }
  ...
  container.querySelectorAll('[data-role="cancel"]').forEach(btn => {
    btn.addEventListener('click', () => { refListEditingId = null; refListEditingCustomerImage = null; refListEditingLocations = []; renderRefListItems(); });
  });
  container.querySelectorAll('.reflist-item-name-input, .reflist-item-country-input, .reflist-contact-type-input, .reflist-contact-company-input, .reflist-contact-country-input, .reflist-contact-jobtitle-input, .reflist-contact-department-input, .reflist-contact-email-input, .reflist-contact-phone-input').forEach(inp => {
    inp.addEventListener('keydown', e => {
      ...
      else if(e.key === 'Escape'){ refListEditingId = null; refListEditingCustomerImage = null; refListEditingLocations = []; renderRefListItems(); }
    });
  });
...
const PROJECT_STAGES = ['Requested','Formulating','Sampling','Customer Review','Approved','In Production','On Hold','Cancelled'];
...
function projectHasUpdateThisMonth(p){
  const now = new Date();
...
function migrateMonthlyUpdate(mu){
  return {
    ...mu,
    plan: mu.plan || '',
...
function monthlyUpdateStatus(mu){
...
function getTaskStatus(mu, todayStr){
...
function blankProduct(recipeId){
  const now = Date.now();
  return {
    id: uid(),
    recipeId,
...
function scheduleProjectSave(p){
...
/* ---------- Trial Results (moved out of the recipe page into its own mode
   so a trial can compare up to TRIAL_MAX_PRODUCTS existing recipes side by
   side, Compare-Recipes-style, instead of belonging to just one). ---------- */
let trials = [];
...
// Shared by Test Participants and Cooking Method — both are just a plain
...
// below listens for.
function trialStringListHtml(items, isEditing, inputClass, addRole, placeholder, datalistId){
  const list = items || [];
...
function migrateTrialsFromRecipes(){
...
// to home.
let mainFeatureView = null; // null | 'recipesList' | 'compare' | 'materials' | 'refLists' | 'projects' | 'trials'
let saveTimer = null;
...
/* ---------- Ingredient master library (shared across every recipe, via Firestore) ---------- */
...
function attachMaterialsListener(){
...
function logActivityEvent(type, entityType, entityName, changes){
...
// Trials have no meaningful set of scalar fields of their own (they're just
// a comparison of recipes/manual products) — "label" stands in for "which
// recipes/products this trial is comparing" as the one thing worth diffing.
...
function snapshotMainFields(obj, fieldMap){
  const snap = {};
...
function diffMainFields(before, after, fieldMap){
  if(!before) return [];
...
function metaItemName(entry){
  return typeof entry === 'string' ? entry : (entry.name || '');
}

function attachMetaListsListener(){
...
function findProjectForRecipe(recipeId){
...
function findMaterialByLabel(text){
  const t = (text || '').trim();
  if(!t) return null;
...
function initMuAttachmentPreviewModal(){
...
function countryFlagBadgeHtml(name){
...
function descriptionListHtml(r){
...
function renderBarList(groups, altClass){
  ...
  main.querySelectorAll('.hd2-stat-card').forEach(el => {
    el.addEventListener('click', () => {
      ...
      if(action === 'filter-in-progress' || action === 'filter-in-review'){
        projectColumnFilters = { status: new Set([action === 'filter-in-progress' ? 'In Progress' : 'In Review']) };
        mainFeatureView = 'projects';
        renderMain();
        renderSidebar();
...
function playContentTransition(el){
  if(!el) return;
...
function openRecipeFromDashboard(recipeId){
...
function allIngredientsInPart(part){
  return [...(part.ingredients || []), ...(part.parts || []).flatMap(allIngredientsInPart)];
}
function allIngredientsInRecipe(r){
...
onAuthStateChanged(auth, user => {
  if(user){
    ...
    trials = [];
    loginEvents = [];
    metaLists = { customers: [], destinationCountries: [], salesReps: [], responsiblePersons: [] };
    currentId = null;
    unlockedRecipeId = null;
    appView = 'auth';
  }
